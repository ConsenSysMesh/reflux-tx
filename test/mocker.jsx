...
describe('Basic Mocketh Coverage', function() {
    ...
    it('getTransaction', function(done) {
        var tx = web3.eth.chain.txs[0];
        var txHash = web3.sha3(JSON.stringify(tx));
...
describe('Fork Test', function() {
    ...
    before(function() {
        ...
        txHash = web3.sha3(JSON.stringify(tx));
    });

    it('batch request', function(done) {
        var batch = web3.createBatch();
        var tx = web3.eth.chain.txs[0];
        var hash = web3.sha3(JSON.stringify(tx));
        var received = 0;
        ...
        batch.add(web3.eth.getTransaction.request(hash, handleRequest.bind(this, hash)));
        batch.add(web3.eth.getTransaction.request(hash, handleRequest.bind(this, hash)));
