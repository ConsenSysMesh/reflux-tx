...
module.exports = Reflux.createStore({
  ...
  loadStorage: function loadStorage(cb) {
    localforage.getItem(this.state.genesis).then((function (storage) {
      ...
      if (err && typeof module === "undefined") {
        console.warn("browser storage not supported: ", err, typeof window);
      }
      cb(err);
    });
  },
  ...
  addTxState: function addTxState(accounts, stateObj) {
    ...
    if (!typeState && stateObj.type === "pending") typeState = [stateObj.hash];else if (!typeState) typeState = stateObj.hash;else if (stateObj.type === "pending") typeState.push(stateObj.hash);else {
      return accounts;
    }var i = nonces.length;
    var nonce;
