...
var StateMixinFactory = _interopRequire(require("reflux-state-mixin"));

var _ = _interopRequire(require("lodash"));

var TXActions = _interopRequire(require("./Actions"));
...
module.exports = Reflux.createStore({
      ...
      Object.keys(this.state.accounts).forEach((function (account) {
        ...
        received.forEach((function (stateObj) {
          ...
          if (number - receipt.blockNumber > this.confirmCount) {
            ...
            if (!latestConfirmed || stateObj.nonce > latestConfirmed) latestConfirmed = stateObj.nonce;
          } else {
            if (!latestReceived || stateObj.nonce > latestReceived) latestReceived = stateObj.nonce;
          }
        }).bind(this));
        ...
        if (latestReceived >= 0) newDropped = pending.filter(function (stateObj) {
  ...
  addTxState: function addTxState(accounts, stateObj) {
    ...
    if (!typeState && stateObj.type === "pending") typeState = [stateObj.hash];else if (!typeState) typeState = stateObj.hash;else if (stateObj.type === "pending") typeState.push(stateObj.hash);else throw new Error("A hash " + typeState + " already exists for type " + stateObj.type + " and account " + stateObj.account + ". Adding hash " + stateObj.hash + " failed");
    ...
    do {
      i--;
      if (i < nonces.length) nonce = nonces[i];
    } while (i > 0 && nonce > stateObj.nonce);

    if (nonce === stateObj.nonce && stateObj.type !== "pending") throw new Error(stateObj.type + " state at nonce " + nonce + " not cleaned up correctly for account " + stateObj.account);

    if (nonce !== stateObj.nonce) nonces.splice(i + 1, 0, stateObj.nonce);
  ...
  delTxState: function delTxState(accounts, stateObj) {
    var save = arguments[2] === undefined ? true : arguments[2];
    ...
    if (typeState.constructor === Array) {
      ...
      if (stateIndex >= 0) typeState.splice(stateIndex, 1);
  ...
  getTxState: function getTxState(hash) {
    ...
    for (var stateIndex = 0; stateIndex < states.length; stateIndex++) {
      ...
      if (!(baseState.nonce in nonces)) continue;
  ...
  getTxStates: function getTxStates() {
    ...
    if (!accounts.length) accounts = Object.keys(_accounts);
  ...
  loadTxData: function loadTxData(payload) {
    ...
    if (!txs.length) txs = this.getTxStates();
    ...
    txs.forEach((function (stateObj) {
      ...
      if (typeof stateObj === "object") hash = stateObj.hash;else stateObj = {
        hash: hash
      };

      hash = utils.formatHex(hash, true);

      if (methodTest.object) batch.add(web3.eth.getTransaction.request(hash, handleData.bind(this, stateObj)));

      if (methodTest.receipt) batch.add(web3.eth.getTransactionReceipt.request(hash, handleReceipt.bind(this, stateObj)));
    }).bind(this));
  ...
  garbageCollect: function garbageCollect() {
    ...
    Object.keys(this.state.accounts).forEach((function (account) {
      ...
      while (confirmed.length + failed.length > this.bufferSize) {
        ...
        if (lastFailed >= 0 && lastFailed < lastConfirmed) removeFromArray = failed;
  ...
  onAdd: function onAdd(payload, cb) {
    ...
    if (newHashes.length) this.loadTxData(newHashes, ["object"], (function (err) {
      if (err) {
        this.setState({ error: err });
        if (typeof cb == "function") cb(err);else console.error("Error in loadTxData", err);
      } else {
    ...
    }).bind(this));else this.saveStorage();
  },
  ...
  recordBlock: function recordBlock(blockId, callback) {
    ...
    getBlockNumber((function (err, number) {
      ...
      web3.eth.getBlock(number, (function (err, block) {
        if (err) this.setState({ error: err });else this.setState({ blockNumber: block.number, timestamp: block.timestamp, blockHash: block.hash });

        if (typeof callback === "function") callback(err, block);
      }).bind(this));
    }).bind(this));
  },
  ...
  */
  onConnect: function onConnect(localWeb3, opts) {
    ...
    txStore.setGenesis(function (err, block) {
      ...
      txStore.loadStorage(function () {
        ...
        didFork(txStore.state.blockHash, function (forked) {
          ...
          if (forked) txsNeeded = unconfirmed;
