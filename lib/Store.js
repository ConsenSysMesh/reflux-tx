...
var web3 = _interopRequire(require("web3"));
...
module.exports = Reflux.createStore({
  ...
  loadStorage: function loadStorage(cb) {
    ...
    var promises = storedKeys.map(function (k) {
      return localforage.getItem(k);
    });
    var defaults = storedKeys.map((function (k) {
      return this.getInitialState()[k];
    }).bind(this));
  ...
  loadTxData: function loadTxData(payload) {
    ...
    if (!txs.length) txs = _.get(this.state.txs, _genesis, []);
    if (!txs.length) {
      return;
    ...
    function handleReceipt(hash, err, recpt) {
      ...
      } else unconfirmed.splice(unconfdex, 1);
  ...
  removeExcessiveTXs: function removeExcessiveTXs() {
    ...
    if (txIndex) this.setState({ info: _info, txs: _txs });
  },
  ...
  newBlock: function newBlock(err, hash) {
    web3.eth.getBlock(hash, (function (err, block) {
      ...
      if (!pending.length && !unconfirmed.length) this.stopWatching();else this.loadTxData(reloadTxs);
    }).bind(this));
  },
  ...
  onAdd: function onAdd(payload) {
    ...
    payload = this.toArr(payload).filter(function (el) {
  ...
  recordBlock: function recordBlock(blockNum) {
